...
namespace BDMPro.Utils
{
    public class Util : IDisposable
    {
        ...
        public bool SupplierNameExists(string suppliername, string currentRecordId)
        {
            bool suppliernameExist = false;
            if (string.IsNullOrEmpty(currentRecordId))
            {
                suppliernameExist = db.Suppliers.Where(a => a.SupplierName == suppliername).Any();
            }
            else
            {
                suppliernameExist = db.Suppliers.Where(a => a.SupplierName == suppliername && a.SupplierId != currentRecordId).Any();
            }
            return suppliernameExist;
        }
        ...
        public List<SelectListItem> GetRolesForMultiSelect(List<string> selectedRoleNames)
        {
            ...
            return list;
                ...
                public List<SelectListItem> GetContactsForMultiSelect(List<string> selectedContactNames)
        {
            List<SelectListItem> list = new List<SelectListItem>();
            if (selectedContactNames != null)
            {
                list = (from t1 in db.Contacts
                        orderby t1.ContactName
                        select new SelectListItem
                        {
                            Text = t1.ContactName,
                            Value = t1.ContactName,
                            Selected = selectedContactNames.Contains(t1.ContactName) ? true : false
                        }).ToList();
            }
            else
            {
                list = (from t1 in db.C
                        orderby t1.Name
                        select new SelectListItem
                        {
                            Text = t1.Name,
