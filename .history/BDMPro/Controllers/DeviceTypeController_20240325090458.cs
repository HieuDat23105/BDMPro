...
public IActionResult Edit(string Id)
{
    ...
    else
    {
        ...
        string maxOrder = db.DeviceTypes.Select(a => a.TypeSymbol).OrderByDescending(a => a).FirstOrDefault();
        int maxOrderInt = maxOrder != null ? int.Parse(maxOrder) : 0;
        model.TypeSymbol = (maxOrderInt + 1).ToString();
    }
    return View(model);
}
        ...
        public void SaveRecord(DeviceTypeViewModel model)
        {
            if (model != null)
            {
                ...
                if (model.Id != null)
                {
                    GlobalOptionSet deviceType = db.GlobalOptionSets.Where(a => a.Id == model.Id).FirstOrDefault();
                    deviceType.Code = sWhitespace.Replace(model.DisplayName, "");
