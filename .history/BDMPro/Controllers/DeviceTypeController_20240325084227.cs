...
namespace BDMPro.Controllers
{
    ...
    public class DeviceTypeController : Controller
    {
        ...
        [CustomAuthorizeFilter(ProjectEnum.ModuleCode.Device, "true", "", "", "")]
        ...
        [HttpPost]
        public async Task<IActionResult> GetPartialViewDevice([FromBody] dynamic requestData)
        {
            try
            {
                ...
                {
                    sort = DeviceListConfig.DefaultSortOrder;
                }
                headers = ListUtil.GetColumnHeaders(DeviceListConfig.DefaultColumnHeaders, sort);
                var list = ReadDevicees();
                string searchMessage = DeviceListConfig.SearchMessage;
                list = DeviceListConfig.PerformSearch(list, search);
                list = DeviceListConfig.PerformSort(list, sort);
                ViewData["CurrentSort"] = sort;
                ViewData["CurrentPage"] = pg ?? 1;
                ViewData["CurrentSearch"] = search;
                int? total = list.Count();
                int? defaultSize = DeviceListConfig.DefaultPageSize;
                ...
                return PartialView("~/Views/Device/_MainList.cshtml", result);
            }
            catch (Exception ex)
            {
