    ...
    public class DeviceTypeController : Controller
    {
        ...
        public DeviceTypeViewModel GetViewModel(string Id, string type)
        {
            ...
            try
            {
                ...
                model.TypeName = device.DisplayName;
                model.Status = device.Status;
                model.OptionOrder = device.OptionOrder;
                model.IsoUtcCreatedOn = device.IsoUtcCreatedOn;
                model.IsoUtcModifiedOn = device.IsoUtcModifiedOn;
                model.SystemDefault = device.SystemDefault;
                if (type == "View")
                {
                    model.CreatedAndModified = util.GetCreatedAndModified(device.CreatedBy, device.IsoUtcCreatedOn, device.ModifiedBy, device.IsoUtcModifiedOn);
                }
            }
            catch (Exception ex)
            {
        ...
        public void SaveRecord(DeviceTypeViewModel model)
        {
            if (model != null)
            {
                ...
                if (model.Id != null)
                {
                    GlobalOptionSet device = db.GlobalOptionSets.Where(a => a.Id == model.Id).FirstOrDefault();
                    device.Code = sWhitespace.Replace(model.DisplayName, "");
                    device.DisplayName = model.DisplayName;
                    device.OptionOrder = model.OptionOrder;
                    device.Type = "UserStatus";
                    device.Status = "Active";
                    device.ModifiedBy = _userManager.GetUserId(User);
                    device.ModifiedOn = util.GetSystemTimeZoneDateTimeNow();
                    device.IsoUtcModifiedOn = util.GetIsoUtcNow();
