...
{
    // Sắp xếp theo chữ cái đầu tiên của TypeSymbol
    string maxOrder = db.DeviceTypes
        .Where(a => a.TypeSymbol.All(char.IsLetter)) // chỉ chọn các TypeSymbol không chứa số
        .Select(a => a.TypeSymbol)
        .OrderByDescending(a => a[0]) // sắp xếp dựa trên chữ cái đầu tiên
        .FirstOrDefault();
