...
namespace BDMPro.Controllers
{
    ...
    {
        private readonly UserManager<Suppliers> _userManager;
        private readonly SignInManager<Suppliers> _signInManager;
        ...
        public SupplierController(DefaultDBContext _db, UserManager<Suppliers> userManager,
                              SignInManager<Suppliers> signInManager, Util _util, ErrorLoggingService logger, IConfiguration configuration, IWebHostEnvironment _environment)
        {
            db = _db;
        ...
        {
            ViewData["SupplierStatusSelectList"] = util.GetDataForDropDownList("", db.GlobalOptionSets, a => a.DisplayName, a => a.DisplayName, a => a.Type == "SupplierStatus");
        ...
        public void SetupSupplierSelectLists(SupplierViewModel model)
        {
            ...
            model.ContactSelectList = util.GetDataForDropDownList(model.ContactId, db.Contacts, a => a.ContactName, a => a.ContactId, a => a.IsDeleted == false);
        }
        ...
        [CustomAuthorizeFilter(ProjectEnum.ModuleCode.SupplierManagement, "", "true", "true", "")]
        public IActionResult Edit(string SupplierId)
        {
            SupplierViewModel model = new SupplierViewModel();
            if (SupplierId != null)
            {
                model = GetSupplierViewModel(SupplierId, "Edit");
            }
            return View(model);
        }
        [CustomAuthorizeFilter(ProjectEnum.ModuleCode.SupplierManagement, "true", "", "", "")]
            ...
            if (model != null)
            {
                ...
                try
                {
                    ...
                    if (model.SupplierId != null)
                    {
                        ...
                        supplỉe.UserName = model.Username;
                        supplỉe.Email = model.EmailAddress;
                        db.Entry(supplỉe).State = EntityState.Modified;
                    ...
                    {
                        type = "create";
                        var user = new Suppliers { UserName = model.Username, Email = model.EmailAddress };
                ...
                catch (Exception ex)
                {
                    if (type == "create")
                    {
                        ...
                        {
                            Suppliers aspNetUsers = db.Suppliers.FirstOrDefault(a => a.Id == userId);
                            if (aspNetUsers != null)
                            {
                                db.Suppliers.Remove(aspNetUsers);
                                db.SaveChanges();
                            }
                        }
