    ...
    public class DeviceTypeController : Controller
    {
        ...
        public DeviceTypeViewModel GetViewModel(string Id, string type)
        {
            ...
            try
            {
                ...
                model.TypeName = .DisplayName;
                model.Status = .Status;
                model.OptionOrder = .OptionOrder;
                model.IsoUtcCreatedOn = .IsoUtcCreatedOn;
                model.IsoUtcModifiedOn = .IsoUtcModifiedOn;
                model.SystemDefault = .SystemDefault;
                if (type == "View")
                {
                    model.CreatedAndModified = util.GetCreatedAndModified(.CreatedBy, .IsoUtcCreatedOn, .ModifiedBy, .IsoUtcModifiedOn);
                }
            }
            catch (Exception ex)
            {
        ...
        public void SaveRecord(DeviceTypeViewModel model)
        {
            if (model != null)
            {
                ...
                if (model.Id != null)
                {
                    GlobalOptionSet  = db.GlobalOptionSets.Where(a => a.Id == model.Id).FirstOrDefault();
                    .Code = sWhitespace.Replace(model.DisplayName, "");
                    .DisplayName = model.DisplayName;
                    .OptionOrder = model.OptionOrder;
                    .Type = "UserStatus";
                    .Status = "Active";
                    .ModifiedBy = _userManager.GetUserId(User);
                    .ModifiedOn = util.GetSystemTimeZoneDateTimeNow();
                    .IsoUtcModifiedOn = util.GetIsoUtcNow();
