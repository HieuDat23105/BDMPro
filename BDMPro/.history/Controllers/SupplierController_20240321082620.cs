    ...
    public class SupplierController : Controller
    {
        ...
        public async Task<IActionResult> GetPartialViewSupplier([FromBody] dynamic requestData)
        {
            try
            {
                ...
                var list = ReadSupplierList(); // Truy vấn dữ liệu từ cơ sở dữ liệu
                ...
                int? total = await list.CountAsync(); // Sử dụng CountAsync để đếm số lượng bản ghi từ cơ sở dữ liệu
            ...
            try
            {
                var supplierList = from t1 in _db.Suppliers.AsNoTracking()
                                   select new SupplierViewModel
                                   {
                                       ...
                                       CreatedOn = t1.CreatedOn.ToString(// Thêm CreatedOn vào danh sách thuộc tính trả về
                                   };
                return supplierList;
            }
            catch (Exception ex)
