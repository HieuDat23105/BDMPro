...
namespace BDMPro.Controllers
{
    ...
    public class SupplierController : Controller
    {
                        ...
                        if (errors.Count == 0)
                        {
                            for (int i = 0; i < dtRowsCount; i++)
                            {
                                try
                                {
                                    ...
                                    su question = new Question();
                                    su.Id = Guid.NewGuid().ToString();
                                    su.QuestionTitle = questionTitle;
                                    su.SubjectId = subjectId;
                                    su.QuestionTypeId = questionTypeId;
                                    su.IsActive = status == "Active" ? true : false;
                                    su.CreatedBy = _userManager.GetUserId(User);
                                    su.CreatedOn = util.GetSystemTimeZoneDateTimeNow();
                                    su.IsoUtcCreatedOn = util.GetIsoUtcNow();
                                    suions.Add(question);
                                    db.SaveChanges();
                                    successCount++;
                                    ModelState.Clear();
        ...
        public void ValidateSupplierModel(SupplierViewModel model)
        {
            if (model != null)
            {
                bool emailExist = util.EmailExists(model.Email Address, model.SupplierId);
                if (emailExist)
                {
        ...
        public void AssignSupplierValues(Supplier supplier, SupplierViewModel model)
        {
            ...
            supplier.Email Address = model.Email Address;
