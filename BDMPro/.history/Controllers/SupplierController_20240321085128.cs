...
namespace BDMPro.Controllers
{
    ...
    public class SupplierController : Controller
    {
        ...
        public SupplierController(DefaultDBContext db, UserManager<AspNetUsers> userManager,
                                  SignInManager<AspNetUsers> signInManager, _Util _util, ErrorLoggingService logger, IConfiguration configuration, IWebHostEnvironment environment)
        {
            _db = db;
        ...
        public async Task<IActionResult> GetPartialViewSupplier([FromBody] dynamic requestData)
        {
            try
            {
                ...
                headers = List_Util.GetColumnHeaders(SupplierListConfig.DefaultColumnHeaders, sort);
...
public SupplierViewModel GetSupplierViewModel(string Id, string type)
                        ...
                        if (errors.Count == 0)
                        {
                            for (int i = 0; i < dtRowsCount; i++)
                            {
                                try
                                {
                                    ...
                                    // errors = _util.ValidateImportSupplierFromExcel(supplierModel);
        ...
        public void AssignSupplierValues(Supplier supplier, SupplierViewModel model)
        {
            ...
            if (model.SupplierId == null)
            {
                ...
                supplier.CreatedOn = _util.GetSystemTimeZoneDateTimeNow();
                supplier.IsoUtcCreatedOn = _util.GetIsoUtcNow();
            }
            else
            {
                supplier.ModifiedBy = model.ModifiedBy;
                supplier.ModifiedOn = _util.GetSystemTimeZoneDateTimeNow();
                supplier.IsoUtcModifiedOn = _util.GetIsoUtcNow();
            }
        }
