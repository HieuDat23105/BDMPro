...
namespace BDMPro.Controllers
{
    ...
    public class SupplierController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        [CustomAuthorizeFilter(ProjectEnum.ModuleCode.Supplie, "true", "", "", "")]
        ...
        public async Task<IActionResult> GetPartialViewLoginHistories([FromBody] dynamic requestData)
        {
            try
            {
                ...
                {
                    sort = SupplieListConfig.DefaultSortOrder;
                }
                headers = ListUtil.GetColumnHeaders(SupplieListConfig.DefaultColumnHeaders, sort);
                var list = ReadLoginHistories();
                string searchMessage = SupplieListConfig.SearchMessage;
                list = SupplieListConfig.PerformSearch(list, search);
                list = SupplieListConfig.PerformSearchDateTime(list, datevalue, offset);
                list = SupplieListConfig.PerformSort(list, sort);
                ViewData["CurrentSort"] = sort;
                ViewData["CurrentPage"] = pg ?? 1;
                ViewData["CurrentSearch"] = search;
                int? total = list.Count();
                int? defaultSize = SupplieListConfig.DefaultPageSize;
                size = size == 0 || size == null ? (defaultSize != -1 ? defaultSize : total) : size == -1 ? total : size;
                ViewData["CurrentSize"] = size;
                PaginatedList<SupplieViewModel> result = await PaginatedList<SupplieViewModel>.CreateAsync(list, pg ?? 1, size.Value, total.Value, headers, searchMessage);
                return PartialView("~/Views/Supplie/_MainList.cshtml", result);
            }
            catch (Exception ex)
            {
        ...
        public IQueryable<SupplieViewModel> ReadLoginHistories()
        {
