...
public async Task<IActionResult> GetPartialViewSupplier([FromBody] dynamic requestData)
{
    try
    {
        ...
        IQueryable<SupplierViewModel> queryableList = list.AsQueryable();

        PaginatedList<SupplierViewModel> result = await PaginatedList<SupplierViewModel>.CreateAsync(queryableList, pg ?? 1, size.Value, total.Value, headers, searchMessage);
