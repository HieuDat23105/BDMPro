...
namespace BDMPro.Controllers
{
    ...
    public class SupplierController : Controller
    {
        ...
        public IQueryable<SupplierViewModel> ReadSupplierList()
        {
            try
            {
                var supplierList = from t1 in db.Suppliers.AsNoTracking()
                                   ...
                                   let t2 = db.GlobalOptionSets.Where(g => g.Id == t1.UserStatusId).SingleOrDefault()
        ...
        {
            model.SupplierSelectList = _util.GetDataForDropDownList(model.SupplierStatusId, db.GlobalOptionSets, a => a.DisplayName, a => a.Id, a => a.Type == "SupplierStatus", a => a.OptionOrder, "asc");
        }
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    else
                    {
                        ...
                        supplier = db.Suppliers.FirstOrDefault(s => s.SupplierId == model.SupplierId);
                        if (supplier == null)
                        {
                            return NotFound();
                        }
