    ...
    public class SupplierController : Controller
    {
        ...
        public async Task<IActionResult> GetPartialViewSupplier([FromBody] dynamic requestData)
        {
            try
            {
                ...
                var list = ReadSupplierList(); // Truy vấn dữ liệu từ cơ sở dữ liệu ở đây
                ...
                int? total = await list.CountAsync(); // Sử dụng CountAsync để đảm bảo việc đếm dữ liệu được thực hiện bằng cách không chờ đợi
        ...
        public IQueryable<SupplierViewModel> ReadSupplierList()
        {
            ...
            {
                _logger.LogError(ex, $"{GetType().Name} Controller - {MethodBase.GetCurrentMethod().Name} Method");
                throw; // Đảm bảo rằng lỗi được ném lại để có thể xử lý ở nơi gọi
            }
        }
