                        ...
                        if (errors.Count == 0)
                        {
                            for (int i = 0; i < dtRowsCount; i++)
                            {
                                try
                                {
                                    ...
                                    var user = new Supplier { SupplierName = upModel.SupplierName, Email = upModel.EmailAddress };
                                    var creationResult = await _userManager.CreateAsync(suppliers, upModel.Password); //create user and save in db
