                ...
                public async Task<bool> SaveRecord(SupplierViewModel model)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(model.SupplierId))
                {
                    ...
                    Supplier existingSupplier = await de.Suppliers.FirstOrDefaultAsync(s => s.SupplierId == model.SupplierId);
                    if (existingSupplier != null)
                    {
                        AssignSupplierValues(existingSupplier, model);
                        de.Entry(existingSupplier).State = EntityState.Modified;
