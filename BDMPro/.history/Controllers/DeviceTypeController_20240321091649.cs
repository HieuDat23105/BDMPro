...
namespace BDMPro.Controllers
{
    ...
    public class DeviceTypeController : Controller
    {
        ...
        public IQueryable<GlobalOptionSetViewModel> ReadDeviceTypees()
        {
        ...
        public IActionResult Edit(string Id)
        {
            ...
            else
            {
                ...
                int? maxOrder = db.GlobalOptionSets.Where(a => a.Type == "DeviceType" && a.Status == "Active").Select(a => a.OptionOrder).OrderByDescending(a => a.Value).FirstOrDefault();
        ...
        public void ValidateModel(GlobalOptionSetViewModel model)
        {
            if (model != null)
            {
                ...
                if (model.Id != null)
                {
                    duplicated = db.GlobalOptionSets.Where(a => a.DisplayName == model.DisplayName && a.Type == "DeviceType" && a.Id != model.Id).Any();
                }
                else
                {
                    duplicated = db.GlobalOptionSets.Where(a => a.DisplayName == model.DisplayName && a.Type == "DeviceType").Select(a => a.Id).Any();
                }
