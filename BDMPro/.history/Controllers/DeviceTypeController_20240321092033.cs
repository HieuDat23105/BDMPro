    ...
    public class DeviceTypeController : Controller
    {
        ...
        public void SaveRecord(DeviceTypeViewModel model)
        {
            if (model != null)
            {
                ...
                if (model.Id != null)
                {
                    DeviceType de = db.DeviceTypes.Where(a => a.Id == model.Id).FirstOrDefault();
                    de.Code = sWhitespace.Replace(model.DisplayName, "");
                    de.DisplayName = model.DisplayName;
                    de.OptionOrder = model.OptionOrder;
                    de.Type = "DeviceType";
                    de.Status = "Active";
                    de.ModifiedBy = _userManager.GetUserId(User);
                    de.ModifiedOn = util.GetSystemTimeZoneDateTimeNow();
                    de.IsoUtcModifiedOn = util.GetIsoUtcNow();
