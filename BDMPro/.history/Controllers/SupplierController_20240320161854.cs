    ...
    public class SupplierController : Controller
    {
        ...
        public async Task<IActionResult> GetPartialViewSupplier([FromBody] dynamic requestData)
        {
            try
            {
                ...
                {
                    sort = istConfig.DefaultSortOrder;
                }
                headers = ListUtil.GetColumnHeaders(istConfig.DefaultColumnHeaders, sort);
                var list = ReadSystemRoles();
                string searchMessage = istConfig.SearchMessage;
                list = istConfig.PerformSearch(list, search);
                list = istConfig.PerformSort(list, sort);
                ViewData["CurrentSort"] = sort;
                ViewData["CurrentPage"] = pg ?? 1;
                ViewData["CurrentSearch"] = search;
                int? total = list.Count();
                int? defaultSize = istConfig.DefaultPageSize;
