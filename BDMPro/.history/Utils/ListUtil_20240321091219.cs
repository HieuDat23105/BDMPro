...
namespace BDMPro.Utils
{
    ...
    {
        public static readonly List<string> TableColumns = new List<string>() {
                nameof(DevViewModel.OptionOrder),
                nameof(DevViewModel.DisplayName),
                nameof(DevViewModel.Actions)
            };

        //searchMessage = the placeholder for the search bar, from here, you can set different placeholder for different listconfig
        public static string SearchMessage = $"{Resource.Search}...";
        public static IQueryable<DevViewModel> PerformSearch(IQueryable<DevViewModel> list, string search)
        {
            if (!string.IsNullOrEmpty(search))
            {
                //perform search
        ...
        public static string DefaultSortOrder = $"{nameof(DevViewModel.OptionOrder)}-asc";
        public static int? DefaultPageSize = 10;
        public static List<SelectListItem> PageSizeDropDownList = ListUtil.GetPageSizeDropDownList(DefaultPageSize);
        public static readonly ColumnHeader[] DefaultColumnHeaders = ListUtil.GenerateDefaultColumnHeaders<DevViewModel>(DefaultSortOrder, TableColumns);

        public static IQueryable<DevViewModel> PerformSort(IQueryable<DevViewModel> list, string sort)
        {
            var result = ListUtil.PerformSort<DevViewModel>(list, DefaultSortOrder, sort);
            return result;
        }
    }
