...
namespace BDMPro.Utils
{
    ...
    {
        public static readonly List<string> TableColumns = new List<string>() {
                nameof(DeViewModel.OptionOrder),
                nameof(DeViewModel.DisplayName),
                nameof(DeViewModel.Actions)
            };

        //searchMessage = the placeholder for the search bar, from here, you can set different placeholder for different listconfig
        public static string SearchMessage = $"{Resource.Search}...";
        public static IQueryable<DeViewModel> PerformSearch(IQueryable<DeViewModel> list, string search)
        {
            if (!string.IsNullOrEmpty(search))
            {
                //perform search
        ...
        public static string DefaultSortOrder = $"{nameof(DeViewModel.OptionOrder)}-asc";
        public static int? DefaultPageSize = 10;
        public static List<SelectListItem> PageSizeDropDownList = ListUtil.GetPageSizeDropDownList(DefaultPageSize);
        public static readonly ColumnHeader[] DefaultColumnHeaders = ListUtil.GenerateDefaultColumnHeaders<DeViewModel>(DefaultSortOrder, TableColumns);

        public static IQueryable<DeViewModel> PerformSort(IQueryable<DeViewModel> list, string sort)
        {
            var result = ListUtil.PerformSort<DeViewModel>(list, DefaultSortOrder, sort);
            return result;
        }
    }
