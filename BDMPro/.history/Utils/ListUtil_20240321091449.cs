...
namespace BDMPro.Utils
{
    ...
    public class DeviceTypeListConfig
    {
        public static readonly List<string> TableColumns = new List<string>() {
                ...
                nameof(DeviceTypeViewModel.CreatedOn),reatedOn),
        ...
        public static IQueryable<DeviceTypeViewModel> PerformSearch(IQueryable<DeviceTypeViewModel> list, string search)
        {
            if (!string.IsNullOrEmpty(search))
            {
                list = list.Where(s => s.Ty.ToString().Contains(search) || s.DisplayName.Contains(search));
            }
            return list;
        }

        public static string DefaultSortOrder = $"{nameof(DeviceTypeViewModel.OptionOrder)}-asc";
        public static int? DefaultPageSize = 10;
        public static List<SelectListItem> PageSizeDropDownList = ListUtil.GetPageSizeDropDownList(DefaultPageSize);
