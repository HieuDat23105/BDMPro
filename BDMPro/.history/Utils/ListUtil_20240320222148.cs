...
namespace BDMPro.Utils
{
    ...
    public class SupplierListConfig
    {
        public static readonly List<string> TableColumns = new List<string>() {
                ...
                nameof(SupplierViewModel.SupplierStatus),
        ...
        public static IQueryable<SupplierViewModel> PerformSearch(IQueryable<SupplierViewModel> list, string search)
        {
            if (!string.IsNullOrEmpty(search))
            {
                list = list.Where(s => s.SupplierName.Contains(search) ||
                                       ...
                                       s.SupplierStatus.Contains(search) ||
        ...
        public static int? DefaultPageSize = 10;
       public static List<SelectListItem> PageSizeDropDownList = ListUtil.GetPageSizeDropDownList(DefaultPageSize);
        public static readonly ColumnHeader[] DefaultColumnHeaders = ListUtil.GenerateDefaultColumnHeaders<SupplViewModel>(DefaultSortOrder, TableColumns);

        public static IQueryable<SupplViewModel> PerformSort(IQueryable<SupplViewModel> list, string sort)
        {
            var result = ListUtil.PerformSort<SupplViewModel>(list, DefaultSortOrder, sort);
            return result;
        }
    }
