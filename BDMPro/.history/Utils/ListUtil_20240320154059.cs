...
public class SupplierListConfig
    {
        ...
        public static IQueryable<SupplierViewModel> PerformSearch(IQueryable<SupplierViewModel> list, string search)
        {
            if (!string.IsNullOrEmpty(search))
            {
                list = list.Where(s => s.SupplierName.Contains(search) || s.Email.Contains(search) || s.Phone.Contains(search) || s.SupplierStatus.Contains(search)) || s.ContactId.Contains(search);;
            }
            return list;
        }
